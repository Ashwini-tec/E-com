...
/********* get all the review according to product ************/
exports.reviewOfProduct= { 
  description: 'filter review according to product',
  auth: false,
  validate: {
      params : Joi.object({
        id: Joi.string().required(),
  ...
  handler:async( request , h )=>{
    try {
        const id = request.params.id ;
        const data = await services.reviewOfProduct(id);
        if(data.err){ return h.response({ message : data.err }).code(400)};
